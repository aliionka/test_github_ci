...
class ClientFactory(SQLAlchemyModelFactory):
    ...
    class Meta:
        ...
        sqlalchemy_session_persistence = 'flush' # чтобы ClientFactory.create() автоматически сохранял объект
    ...
    credit_card = LazyAttribute(
        lambda _: random.choice([None, factory.Faker('credit_card_number').evaluate(None, None, {'locale': None})])
...
class ParkingFactory(SQLAlchemyModelFactory):
    ...
    count_available_places = factory.LazyAttribute(lambda x: x.count_places) # зависит от других полей того же объекта