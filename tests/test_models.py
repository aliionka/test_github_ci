...
from tests.conftest import sample_client, sample_parking, full_parking, closed_parking, db_session
from app.views import calculate_cost, process_payment
...
class TestGetEndpoints:
    ...
    def test_get_client_invalid_id_type(self, client):
        ...
        response = client.get(f'/api/client/lala')
        assert response.status_code == 404
...
class TestCreate:
    ...
    @pytest.mark.parametrize('data', [
        {'name': 'Bob', 'surname': 'Marli', 'car_number': 'C333CC', 'credit_card': '0987-6543-2109-8765'},
        {'name': 'Lady', 'surname': 'Gaga', 'car_number': 'E555EE', 'credit_card': '2109-8765-0987-6543'}
    ])
    ...
    @pytest.mark.parametrize('data', [
        {'name': None, 'surname': 'Petrov', 'car_number': 'C333CC', 'credit_card': '0987-6543-2109-8765'},
        {'name': 'Lady', 'surname': None, 'car_number': 'E555EE', 'credit_card': '2109-8765-0987-6543'},
    ])
    ...
    @pytest.mark.parametrize('data', [
        {'address': 'St. Petersburg, Nevsky avenue, 1', 'opened': True, 'count_places': 28,
         'count_available_places': 14},
        {'address': 'St. Petersburg, Nevsky avenue, 2', 'opened': False, 'count_places': 34,
...
class TestParkingAPI:
    ...
    def test_enter_success(self, client, db_session, sample_client, sample_parking):
        ...
        # Делаем запрос
        response = client.post('/api/client_parkings/enter', json={
            'client_id': sample_client.id,
    ...
    def test_exit_success(self, client, db_session, active_parking_log):
        ...
        # Делаем запрос
        response = client.delete('/api/client_parkings/exit', json={
        ...
        duration_minutes = (active_parking_log.time_out - active_parking_log.time_in).total_seconds() / 60
