...
def create_parking():
    ...
    try:
        ...
        # Валидация обязательных полей
        if not parking_data.get('address') or not parking_data.get('count_places') or not parking_data.get('count_available_places'):
            return jsonify({'error': 'Address, count places and count available places are required'}), 400
...
def process_payment(credit_card, amount):
    ...
    if not credit_card:
        return PaymentResult(False, f'Credit card is required')

    if amount < 0:
        return PaymentResult(False, f'Amount must be positive')

    if amount == 0:
        return PaymentResult(True, f'Free parking')
...
def exit_from_the_parking_lot():
    ...
    try:
        ...
        if not client.credit_card:
            return jsonify({'error': 'No credit card linked. Please add payment method'}), 402
